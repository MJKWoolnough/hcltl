...
if [ ! -e data.js ] || [ $(stat -c %s data.js) -ne 61 ]; then
	echo "export const data = [], users = [], alarms = [], lines = [];" > data.js;
fi;
...
(
	...
	jslib -i /script.js -n -x | tail -n+2 | sed -e 's/pageLoad/(document.readyState == "complete" ? Promise.resolve() : new Promise(successFn => globalThis.addEventListener("load", successFn, {once: true})))/' | terser -m  --module --compress pure_getters,passes=3 --ecma 6 | tr -d '\n' | sed -e 's/\\/\\\\/g' -e 's/"/\\\"/g' -e 's/\(.*=\[\)\(\],[^=]*=\[\)\(\],[^=]*=\[\)\(\],[^=]*=\[\)\]/\1\"'"\n"'	mid   = \"\2\"'"\n"'	mid2  = \"\3\"'"\n"'	mid3  =\"\4\"'"\n"'	end   = \"]/';
